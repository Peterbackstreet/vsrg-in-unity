...
public class beatLineGenerator : MonoBehaviour
{
    ...
    {
        float start_offset = startOffset * 0.001f;
        float Length = audioController.Instance.chartLength - start_offset; 
        ...
        for (float beat = 0; beat < beatCnt; beat++)
        {
            ...
            newBeatLine.GetComponent<BeatLine>().getInstanceValue(BPM, start_offset);
